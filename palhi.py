...
def gen_patient_idx_dict(patient_ID):
...
def load_likelihood_test(llh_file):
    """
...
def gen_likelihood_hist(likelihood, patient_id, num_bin, norm_hist=False):
    """
    ...
    patient_idx_dict, unique_patient_idx = gen_patient_idx_dict(patient_id)
...
def gen_wsi_df_test(te_data, te_unique_patient_idx, te_pred_prob, te_pred_label):
...
def palhi_inference(te_data, clf, num_bin=200, norm_hist=False):
    """ PAtch Likelihood HIstogram pipeline
        ...
        norm_hist:
    """
    te_patient_hist, te_unique_patient_idx = gen_likelihood_hist(te_data['likelihood'], te_data['patient_id'],
                                                                 num_bin, norm_hist)
